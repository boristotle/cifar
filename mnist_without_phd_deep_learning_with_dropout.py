...
DONT USE DROPOUT IN TEST, only in TRAINING
DROPOUT IS APPLIED AFTER EACH LAYER EXCEPT FOR THE SOFTMAX/OUTPUT LAYER
'''
...
Y4d = tf.nn.dropout(Y4, keep_prob=keep_prob)
#model
...
#learning_rate = tf.train.exponential_decay(.003, global_step, 10000, .96)
...
#load batch of images and correct answers in groups of 100
    ...
    acc_training.append(a)
    iteration.append(i)
    #print('i', i)
    if (i == 999):
        print('iteration', iteration)
        plt.plot(iteration, acc_training, "o", color="r", label='training')
    ...
    if (i == 999):
        print('iteration', iteration)
        plt.plot(iteration, acc_test, "o", color="b", label='test')
